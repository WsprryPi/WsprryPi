...
root = pathlib.Path('/home/pi/issue446-rf') / str(freq)
...
report = {'frequency_hz': freq, 'duration_ms': 10000, 'manual_ppm': 0.0, 'binary_sha256': hashlib.sha256(binary.read_bytes()).hexdigest(), 'source_commit': command(['git','-C','/home/pi/WsprryPi','rev-parse','HEAD']), 'config_before_sha256': hashlib.sha256(before).hexdigest(), 'events': []}
...
try:
    ...
    with (root/'transmitter.log').open('w') as log:
        proc = subprocess.Popen(args, cwd=root, stdout=log, stderr=subprocess.STDOUT)
        deadline = time.monotonic()+8
        while True:
            assert proc.poll() is None, 'transmitter process exited during startup'
            try:
                ...
                time.sleep(.1)
        event('request_begin', freq)
        result = run_bounded_tone_transaction(BoundedToneEndpoint('127.0.0.1',41726), request_id='issue446-'+str(freq), frequency_hz=freq, duration_ms=10000, outer_timeout_s=20)
        event('bounded_tone', result)
        success = True
