...
try:
    ...
    args=[capture,'--enable-physical-sdr','sdrplay','2404058C60',str(freq-25000),'11250000','20','250000','200000','0','false','false','500000','55',str(root/'capture.cf32'),str(root/'metadata.json'),'issue446-'+str(freq)]
    ...
    with (root/'capture.log').open('w') as log:
        ...
        time.sleep(4)
        device.set_freq(0,freq+20000,False); device.read(); event('reference_step_high',state())
        assert device.f1==freq+20000 and device.sat_lock and device.pll_lock
        time.sleep(4)
        device.set_freq(0,freq+10000,False); device.read(); event('reference_step_return',state())
        assert device.f1==freq+10000 and device.sat_lock and device.pll_lock
        time.sleep(3)
